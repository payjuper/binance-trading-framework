...
# ========= USER CONFIG =========
SYMBOL        = "BTCUSDT"            # USDT market (change to BTCUSDC if supported)
INTERVAL      = Client.KLINE_INTERVAL_1MINUTE
LOOKBACK      = 140                  # lookback window (minutes)
THRESHOLD     = 0.04                 # enter short if current price rose >= 4% from recent low
...
# ✅ logging interval (print balance & rise ratio)
LOG_INTERVAL  = 60 * 60              # print once per hour
...
def ensure_symbol_exists(symbol):
    ...
    if symbol not in syms:
        raise RuntimeError(f"[CONFIG] Futures symbol '{symbol}' not found.")
...
def get_symbol_filters(symbol):
    ...
    raise RuntimeError(f"[CONFIG] No filters for {symbol}")
...
def current_position_size(symbol):
    ...
    return amt, entry  # <0: short, 0: flat
...
def place_short_market_with_brackets(symbol, qty, entry_price):
    # enter short
...
def main():
    ...
    print(f"[START] {SYMBOL} trading bot (SHORT) | leverage={LEVERAGE}x | margin={MARGIN_TYPE} | testnet={TESTNET}")
    ...
    while True:
        try:
            ...
            rise = (last - recent_low) / recent_low if recent_low > 0 else 0.0
            ...
            if now_ts - last_log_ts >= LOG_INTERVAL:
                print(f"[{now_str()}] ✅ Bot running | rise: {rise*100:.2f}% | balance({quote_asset}): {avail_quote:.2f}")
                last_log_ts = now_ts

            # entry condition (short when price pumped from recent low)
            if not in_position and rise >= THRESHOLD:
                if avail_quote <= 0:
                    ...
                    if qty < min_qty:
                        ...
                        print(f"[{now_str()}] [ENTRY] SHORT {SYMBOL} qty={qty}, price~{last:.2f}, rise={rise*100:.2f}%")
                        tp, sl = place_short_market_with_brackets(SYMBOL, qty, last)
